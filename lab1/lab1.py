...
Rule_1=IF(AND('parent (?x) (?y)','parent (?x) (?z)'),THEN('sibling (?y) (?z)'),DELETE('sibling (?y) (?y)'))
Rule_2=IF(AND('sibling (?x) (?y)','male (?x)'),THEN('brother (?x) (?y)'),DELETE('brother (?y) (?y)'))
Rule_3=IF(AND('sibling (?x) (?y)','female (?x)'),THEN('sister (?x) (?y)',),DELETE('sister (?y) (?y)'))
Rule_4=IF(AND('parent (?x) (?y)','male (?x)','female (?y)'),THEN('father (?x) (?y)','daughter (?y) (?x)'),DELETE('father (?y) (?y)','daughter (?y) (?y)'))
Rule_5=IF(AND('parent (?x) (?y)','male (?x)','male (?y)'),THEN('father (?x) (?y)','son (?y) (?x)'),DELETE('father (?y) (?y)','son (?y) (?y)'))
Rule_6=IF(AND('parent (?x) (?y)','female (?x)','male (?y)'),THEN('mother (?x) (?y)','son (?y) (?x)'),DELETE('mother (?y) (?y)','son (?y) (?y)'))
Rule_7=IF(AND('parent (?x) (?y)','female (?x)','female (?y)'),THEN('mother (?x) (?y)','daughter (?y) (?x)'),DELETE('mother (?y) (?y)','daughter (?y) (?y)'))
Rule_8=IF(AND('parent (?x) (?y)','parent (?z) (?a)','sibling (?x) (?z)'),THEN('cousin (?y) (?a)'),DELETE('cousin (?y) (?y)'))
Rule_9=IF(AND('male (?x)','parent (?x) (?y)','parent (?z) (?x)'),THEN('grandfather (?z) (?y)','grandchild (?y) (?z)'),DELETE('grandfather (?y) (?y)','grandchild (?y) (?y)'))
Rule_9=IF(AND('female (?x)','parent (?x) (?y)','parent (?z) (?x)'),THEN('grandmother (?z) (?y)','grandchild (?y) (?z)'),DELETE('grandmother (?y) (?y)','grandchild (?y) (?y)'))
#Rule_2=IF('sibling (?x) (?x)',THEN('same-identity (?x) (?x)'))
# Then, put them together into a list in order, and call it
# family_rules.
family_rules = [Rule_1,Rule_2,Rule_3,Rule_4,Rule_5,Rule_6,Rule_7,Rule_8,Rule_9]                    # fill me in
...
HOW_MANY_HOURS_THIS_PSET_TOOK = '350'
WHAT_I_FOUND_INTERESTING = 'Forward chaing essentials'
WHAT_I_FOUND_BORING = 'same ideintity thing'
